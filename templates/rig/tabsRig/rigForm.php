...
if (($active_tab == 3 && !isset($new_active_tab)) || (isset($new_active_tab) && $new_active_tab == 30)) {
    ...
    ?>
                ...
                <a href="#" id="validate_time_msg" data-toggle="modal"  data-target="#modal_validate_time_msg" ></a>


<?php
include dirname(__FILE__) . '/modal_validate_time_msg.php';
...
if ($active_tab == 1) {
    ...
    ?>
                        $('form#rigForm').submit(function (e) {

                            // Запрещаем стандартное поведение для кнопки submit
                            e.preventDefault();

                            var time_msg = $('#rigForm [name="time_msg"]').val();

                            if (time_msg == '')
                                toastr.error('Укажите дату и время сообщения', 'Ошибка!', {timeOut: 2500});
                            else {
                                $.ajax({
                                    type: 'POST',
                                    url: '/journal/check_time_msg',
                                    dataType: 'json',
                                    data: {time_msg: time_msg},
                                    success: function (data) {
                                        if (typeof (data.error) != "undefined") {
                                            if (data.is_show_modal === 1) {
                                                $('#validate_time_msg').click();
                                                $('#modal_validate_time_msg .modal-body p').text(data.error);

                                            } else
                                                toastr.error(data.error, 'Ошибка!', {timeOut: 2500});
                                        } else {
                                            var form = $('form#rigForm');
...
?>
                    ...
                    $('body').on('click', '#modal_validate_time_msg #btn_save_rig', function () {

                        $('#modal_validate_time_msg').click();
                        var form = $('form#rigForm');
                        form.unbind('submit').submit();
