...
class Model_Actionwaybill
{
    ...
    public function selectById($id)
    {
        ...
        return R::getAll('SELECT a.id, r.name as reason_name, a.description,a.is_off, a.ord, a.last_update, r.id as reason_id from journal.actionwaybill as a '
                . 'left join journal.reasonrig as r on a.id_reasonrig=r.id WHERE a.id =? ', array($id));
    }
    ...
    {
         return R::getAll('SELECT a.`description` FROM journal.rig as r left join  journal.actionwaybill as a on a.id_reasonrig=r.id_reasonrig WHERE r.id =? AND a.is_off = ? ORDER BY a.ord ', array($id_rig,1));
    }
        ...
        public function isOrd($id)
    {
        ...
        return R::getAll('SELECT a.ord from journal.actionwaybill as a '
            ...
            public function editOrd($id_reasonrig,$old_ord,$new_ord)
    {

                $id=R::getCell('SELECT id FROM journal.actionwaybill  WHERE id_reasonrig =? and ord = ? LIMIT ? ', array($id_reasonrig,$old_ord,1));

        return R::getAll('UPDATE journal.actionwaybill SET ord = ?  WHERE id = ?  ', array($new_ord,$id));
    }

    public function selectAllActionByIdReason($id_reasonrig)
    {
  return R::getAll('SELECT a.id, r.name as reason_name, a.description,a.is_off, a.ord,a.last_update, r.id as reason_id from journal.actionwaybill as a '
                . 'left join journal.reasonrig as r on a.id_reasonrig=r.id WHERE r.id =?  ORDER BY a.ord ASC', array($id_reasonrig));
    }
