                ...
                $.each(data.images, function (key, value) {
                    ...
                    if ($(e.target).prop('files') !== undefined && $(e.target).prop('files') !== null) {
                        ...
                        if (file) {
                            ...
                            if (value.type_source === 'file') {
                                $el.parent().parent().prepend('<i class="fa fa-file-text-o fa-lg file-icon issue-file-type" style="margin-left: 10px;" data-name="' + value.file_name + '" aria-hidden="true">' + value.file_name + '</i><i class="fa fa-times-circle-o fa-lg delete-icon-file-type" data-name="' + value.file_name + '" aria-hidden="true" style="margin-left: 5px;"></i>');
                            } else {
                                console.log(type);
                                switch (type) {
                                    case 'image':
                                    ...
                                    default:
                                        $el.parent().prepend('<i class="fa fa-file-text-o fa-lg file-icon" data-name= aria-hidden="true">' + value.file_name + '</i>');
                                        break;
                                }
                            }
...
$('body').on('click', '.upload-info-clear-multi', function (e) {
    ...
    if ($(this).hasClass('student-test-upload-info-clear-multi')) {
        ...
        if ($uploadInput.hasClass('video')) {// reset video
            ...
            $uploadInput.parent().find('.delete-icon-file-type').remove();
            ...
            $.each($('.file-icon'), function (key, value) {
                $(this).remove();
            });
        }
    }
...
$('body').on('click', '.delete-img-multi', function (e) {
    ...
    if ($('.issue-file-type').length === 0 && $('.issue-img-multi').length === 0) {
...
$('body').on('click', '.delete-icon-file-type', function (e) {
    ...
    $.each($('.issue-file-type'), function (key, value) {
        if ($(this).attr('data-name') === file) {
            $(this).remove();
